...
*(This is the submission's own documentation. `ASSIGNMENT_BRIEF.md` in this
repository is the original assignment brief itself and has been left
untouched throughout — see "A note on this file" at the bottom. `DESIGN.md`
is the required ≤3-page answer to the brief's three reflective questions;
`ARCHITECTURE.md` is a longer, supplementary architecture reference for
anyone who wants the detail behind the summary given here.)*
...
`ASSIGNMENT_BRIEF.md` in this repository is the original assignment brief
(originally supplied as `README.md`) and has been treated as a read-only
file throughout every phase of this project (verified by checksum at every
phase boundary), then renamed here to make room for this solution
documentation on the repository's landing page. A duplicate of this content
also exists at `SOLUTION_README.md` from earlier in the project's history.
